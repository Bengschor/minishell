...
/*   Updated: 2024/04/26 19:15:31 by bschor           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * @return The length of the replaced substring if successful, otherwise a negative value indicating an error.
 * @line by line:
 * 1-7: Extract the substring to be expanded from the original string, starting from the index after the dollar sign.
 * 8: Search for the corresponding environment variable using the extracted substring.
 * 9: If a matching environment variable is found:
 * 10: Extract the value part of the environment variable.
 * 11: Replace the substring in the original string with the value of the environment variable.
 * 12-14: Free the memory allocated for the extracted substring.
 * 15: Return the length of the replaced substring if successful, or a negative value indicating an error.
 */
...
/**
 * @brief Replaces the "$?" substring in the original string with the exit status of the previous command.
 * 
 ...
 * 4-7: Replace the "$?" substring in the original string with the exit status string.
 * 8: Free the memory allocated for the exit status string.
 * 9: Return the length of the replaced substring if successful, or a negative value indicating an error.
 */
 ...
 * 2-8: Check if the current character indicates an environment variable, exit status, or neither, and expand accordingly.
 * 10-12: Update the index based on the length of the replaced substring or the expanded environment variable.
 */
