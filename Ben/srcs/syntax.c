...
/*   Updated: 2024/05/23 11:13:18 by bschor           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	quotes_by_pair(char *str, t_system *systm)
{
	...
	while (str[i])
	{
		if ((str[i] == SQUOTE || str[i] == DQUOTE) && !str[i + 1])
			return (add_history(systm->prompt), ft_print_syntax("not interpreting unclosed quotes\n", systm));
		if (str[i] == SQUOTE)
		{
...
int	check_syntax(t_system *systm)
{
	...
	cur = systm->lexer;
	if (cur->token == PIPE)
		return (ft_printf_err(TKNSTX, cur->token), ft_print_syntax(NULL, systm));
	while (cur->next)
	{
		if ((cur->token && cur->next->token) && ((ft_strchr("#=<>", cur->token)
			&& ft_strchr("#=<>|", cur->next->token))
			|| cur->token == cur->next->token))
			return (ft_printf_err(TKNSSTX, tkntostr(cur->next->token)), ft_print_syntax(NULL, systm));
		cur = cur->next;
	}
